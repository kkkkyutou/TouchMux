...
3. 前端只需要暴露 Hub。
4. Hub 与各 Node 之间的 HTTP 和终端 WebSocket 握手都通过 `TOUCHMUX_NODE_SHARED_SECRET`、`x-touchmux-node-ts`、`x-touchmux-node-nonce`、`x-touchmux-node-signature` 通信。
...
- `TOUCHMUX_NODE_REQUEST_REPLAY_CACHE_LIMIT`
