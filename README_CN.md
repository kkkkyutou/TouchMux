...
Goal Guard 是 TouchMux 里负责“持续盯住任务直到真的满足停止条件”的那部分。

- 守卫启用时，浏览器侧可以锁住终端文本输入，减少手机端误触。
- 它会把“等待中”“检测到新输出”“自动续跑中”分开处理，不再把这些状态混成一类。
- 现在界面里提供了调试快照面板，可以直接检查：
  - 是否真的命中了成功关键词
  - 是否真的出现了单独一行 `SUCCESS`
  - 守卫实际用于判定的输出窗口末尾
  - 去掉守卫自注入提示后的清洗窗口末尾
  - 用于判断“终端是否变化”的 pane 快照末尾

使用建议：

- 如果任务在你启动守卫之前就已经输出了 `SUCCESS`，现在重新启用守卫也应该能直接识别到这条已有成功。
- 如果怀疑出现误判或漏判，打开 `目标守卫` -> `查看调试快照`，重点对比“原始判定窗口”和“清洗后的判定窗口”。
- 对比较重要的任务，建议在 `SUCCESS` 之外再配合一个更严格的 `successCommand` 做校验。
